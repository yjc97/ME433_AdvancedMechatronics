...
void ssd1306_drawLetter(int x, int y, char letter){
    // draw a letter to the screen based on ASCII value in font.h
    int i = 0, j = 0;
    while (i < 5){
      j = 0;
      while (j < 8){
        if ((ASCII[letter - 0x20][i] >> j) & 1){
...
void ssd1306_drawMessage(int x, int y, char* message){
    // draw an array of ASCII chars sent in a char pointer
    // will move to next lines on ssd if message is too long
    int i = 0, j = 0, k = 8;
    while (message[i] != 0){
      if (x + 5*i >= 123){
        if (j > 123/5){
        ...
        ssd1306_drawLetter(j*5, y + k, message[i]);
        j++;
